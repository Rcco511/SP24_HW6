...
class steam():
    ...
    def calc(self):
        ...
        ts, ps, hfs, hgs, sfs, sgs, vfs, vgs = np.loadtxt("./sat_water_table.txt", delimiter=None, unpack=True, skiprows=1) #use np.loadtxt to read the saturated properties
        tcol, hcol, scol, pcol = np.loadtxt("./superheated_water_table.txt", delimiter=None, unpack=True, skiprows=1) #use np.loadtxt to read the superheated properties
        ...
        if self.T is not None:
            if self.T>Tsat: #interpolate with griddata
                ...
                self.h = float(griddata((self.T, self.p), hcol, self.p, method ='linear'))  #use griddata to interpolate with T & P the superheated table
                self.s = float(griddata((self.T, self.p), scol, self.p, method ='linear'))  #use griddata to interpolate with T & P the superheated table
                self.x=1.0
            ...
            if self.x<=1.0: #manual interpolation
                ...
                self.region='Superheated'
                self.T = float(griddata((self.h, self.p), tcol, self.p, method ='linear'))  #use griddata to interpolate with h & P the superheated table
                self.s = float(griddata((self.h, self.p), scol, self.p, method ='linear'))  #use griddata to interpolate with h & P the superheated table
        elif self.s!=None:
            ...
            if self.x<=1.0: #manual interpolation
                ...
                self.region = 'Superheated'
                self.T = float(griddata((self.s, self.p), tcol, self.p, method ='linear'))  #use griddata to interpolate with s & P the superheated table
                self.h = float(griddata((self.s, self.p), hcol, self.p, method ='linear'))  #use griddata to interpolate with s & P the superheated table
        #endregion
